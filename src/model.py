...
class ClassifierWithEmbedding(torch.nn.Module):
    def __init__(self, inch, act, nchs_orig: list):
        super().__init__()
        assert inch % 27 == 0, "Every variable must have the same dimension (27)"
        embd_ch = (inch // 27) * 8
        self.projector = torch.nn.Sequential(
            torch.nn.Conv1d(inch, embd_ch, 1, groups=inch // 27),
            get_activation(act)
        )
...
class ClassifierWithAttention(torch.nn.Module):
    def __init__(self, inch, act, nchs_orig: list):
        ...
        self.classifier = ClassifierWithBatchNorm(embd_ch // 2 + (inch // 27) ** 2, act, nchs_orig)
        #self.classifier = ClassifierWithDropout(embd_ch, act, nchs_orig)
